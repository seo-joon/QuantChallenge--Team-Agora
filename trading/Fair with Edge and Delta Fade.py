...
class Strategy:
    ...
    def reset_state(self) -> None:
        ...
        self.edge = 1
        self.delta = 0
        self.deltaDampener = 10

    def fair(self):
        logit = np.tan(
            np.pi/2 * 
            (self.home_score-self.away_score+self.delta/self.deltaDampener)/
            (self.home_score+self.away_score))
        time_frac = self.time_left/self.time_start
        prob = 1/(1+np.exp(-logit/time_frac))
        return 100 * prob
    ...
    def on_game_event_update(self,
        ...
        if self.ticker:
            place_limit_order(Side(0), self.ticker, 1, self.fair()-self.edge)
            place_limit_order(Side(1), self.ticker, 1, self.fair()+self.edge)
